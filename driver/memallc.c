...
void* memallc(uint64_t size)
{
    ...
    if (size > range)
    {   
        uart_puts("memallc: Requested size exceeds range. Allocating maximum available block.\r\n");
...
void free_memallc(void* addr)
{
    ...
    {
        uart_puts("memallc: Invalid address or no allocated memory blocks.\r\n");
        return;
    } 
    memblk_t* block = NULL;
    memblk_t* prev = NULL;
    ...
    while (current)
    {
        ...
        {
            uart_puts("memallc: Found allocated block. Freeing it.\r\n");
            block = current;
            break;
        }
        prev = current;
    ...
    if (!block)
    {
        uart_puts("memallc: Address not found in allocated memory blocks.\r\n");
        return;
    }
    if (prev)
    {
    ...
    {
        uart_puts("memallc: Removing block from allocated list. This block is the head of the allocated list.\r\n");
        allc_memblk_list = block->next;
    }
    block->next = free_memblk_list;
    free_memblk_list = block;
    uart_puts("memallc: Block added to free list. Attempting to defragment.\r\n");
...
memblk_t* allc_memblk(uint64_t size)
{
    ...
    {
        uart_puts("memallc: Invalid size or no free memory blocks available.\r\n");
        return NULL;
    }
    memblk_t* prev = NULL;
...
static memblk_t* allc_memblk_impl_split(uint64_t size, memblk_t* prev, memblk_t* block)
{
    if (block->size > size)
    {
        ...
        if (new_block)
        {
            uart_puts("memallc: Splitting free block and creating new block.\r\n");
    ...
    {
        uart_puts("memallc: Removing block from free list. This block is the head of the free list.\r\n");
...
static void defragment_memblk(memblk_t* block)
{
    ...
    while (current)
    {
        ...
        else if ((char*)block->addr + block->size == current->addr)
        {
            ...
            {
                uart_puts("memallc: Current block is head of free list. Updating head.\r\n");
                free_memblk_list = block;
            }
            else if (prev_current)
            {
                uart_puts("memallc: Updating previous block to point to merged block.\r\n");
